                ...
                if line[0] not in vowel:                                                                               
                    if start_char not in vowel:                                                                                           # consonant-to-consonant
                        ...
                        if layout[line[0]][2] == layout[start_char][2] :
                            ...
                            elif calculateDistance(layout[start_char], layout[line[0]]) <= m.sqrt(2):
                    ...
                    if start_char in vowel:                                                                                               # vowel-to-vowel (same)
                        ...
                        if line[0] in vowel_1:                                                                                                      # consonant-to-vowel
                        ...
                        elif line[0] in vowel_2:                                                                                                      # consonant-to-vowel
                            if layout[start_char][2] == 1:                                                                                           # same_layer
                                click = 2
                                if calculateDistance(layout[start_char], layout[line[0]]) == 0:
                                    total_weight = click_depth 
                                    if pair in dict: 
                ...
                if line[i+1] not in vowel:                                                                                             # consonant
                    if line[i] not in vowel:                                                                                           # consonant-to-consonant
                        ...
                        if layout[line[i+1]][2] == layout[line[i]][2]:
                            if calculateDistance(layout[line[i]], layout[line[i+1]]) == 0:
                                total_weight = click_depth 
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, line[i], line[i+1], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, line[i], line[i+1], total_weight)
                            elif calculateDistance(layout[line[i]], layout[line[i+1]]) <= m.sqrt(2):
                    ...
                    if line[i] in vowel:                                                                                               # vowel-to-vowel (same)
                        ...
                        elif line[i+1] in vowel_2:
                            if layout[line[i]][2] == 1:                                                                                  # 1st_layer
                                click = 2
                                if calculateDistance(layout[line[i]], layout[line[i+1]]) == 0:
                                    total_weight = click_depth 
                                    if pair in dict: 
