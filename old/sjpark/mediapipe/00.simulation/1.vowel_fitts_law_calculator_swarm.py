...
keyWidth = 1.0
# SWARM keyboard layout
layout = {'a': (0, 0, 0), 'l': (1, 0, 0), 'b': (2, 0, 0), 'h': (0, 1, 0), 'j': (1, 1, 0), 'c': (2, 1, 0), 'd': (0, 2, 0), 'k': (1, 2, 0),  'g': (2, 2, 0), 'w': (0, 0, 1), 'o': (1, 0, 1), 'e': (2, 0, 1), 'n': (0, 1, 1), 'q': (1, 1, 1), 'i': (2, 1, 1), 'f': (0, 2, 1), 'm': (1, 2, 1), 's': (2, 2, 1), 'z': (0, 0, 2), 't': (1, 1, 2), 'y': (2, 1, 2), 'p': (0, 2, 2), 'r': (1, 2, 2), 'u': (2, 2, 2), 'v': (1, 1, 3), 'x': (1, 2, 3)}
            ...
            if start_char != 0:
                ...
                if line[0] not in vowel:                                                                          # consonant
                    if start_char not in vowel:                                                                   # consonant-to-consonant
                        ...
                        # layer 4
                        elif layout[line[0]][2] == 3:                            
                            if calculateDistance(layout[start_char], layout[line[0]]) == 0:                                             # same_position_key
                                total_weight = click_depth * 4
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, start_char, line[0], dict[pair][3] + total_weight)
                                else:
                                    dict[pair] = (count, start_char, line[0], total_weight)
                            else:
                                total_weight = FittsLaw(keyWidth, calculateDistance(layout[start_char], layout[line[0]])) + click_depth * 4 # different_position_key
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, start_char, line[0], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, start_char, line[0], total_weight)
                        ...
                        if mem_con == 0:
                            ...
                            # layer 2    
                                ...
                                total_weight = FittsLaw(keyWidth, keyWidth*m.sprt(2)) + change_layer + click_depth * 2
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, start_char , line[0], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, start_char , line[0], total_weight)    
                            ...
                            # layer 3    
                                ...
                                total_weight = FittsLaw(keyWidth, keyWidth*m.sprt(2)) + change_layer + click_depth * 3
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, start_char , line[0], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, start_char , line[0], total_weight)
                            ...
                            # layer 4    
                                ...
                                total_weight = FittsLaw(keyWidth, keyWidth*m.sprt(2)) + change_layer + click_depth * 4
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, start_char , line[0], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, start_char , line[0], total_weight)
                            ...
                            # layer 4    
                                ...
                                total_weight = FittsLaw(keyWidth, calculateDistance(layout[mem_con], layout[line[0]])) + change_layer + click_depth * 4
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, start_char , line[0], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, start_char , line[0], total_weight)    
                    ...
                    if line[i] not in vowel:                                                                                           # consonant-to-consonant
                        ...
                        # layer 4    
                            ...
                            if calculateDistance(layout[line[i]], layout[line[i+1]]) == 0:                                             # same_position_key
                                ...
                                total_weight = FittsLaw(keyWidth, calculateDistance(layout[line[i]], layout[line[i+1]])) + click_depth * 4 # different_position_key
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, line[i], line[i+1], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, line[i], line[i+1], total_weight)                
                        ...
                        if mem_con == 0:
                            ...
                            # layer 4
                                ...
                                total_weight = FittsLaw(keyWidth, keyWidth*m.sqrt(2)) + change_layer + click_depth * 4
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, line[i] , line[i+1], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, line[i] , line[i+1], total_weight)
                            ...
                            # layer 4    
                                ...
                                total_weight = FittsLaw(keyWidth, calculateDistance(layout[mem_con], layout[line[i+1]])) + change_layer + click_depth * 4
                                if pair in dict:
                                    dict[pair] = (dict[pair][0] + count, line[i] , line[i+1], dict[pair][3]+total_weight)
                                else:
                                    dict[pair] = (count, line[i] , line[i+1], total_weight)   
...
data_path = 'Ebook4_LesMiserables_original_568,568.txt'
print(data_path)
