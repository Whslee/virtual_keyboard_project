...
def alphabetTable(data_path, layout):
    fp = open(data_path)
    line = fp.readline()
    total = 0  
    count = 1
    ...
    while line:
        ...
        for i in range(len(line)):
            char = line[i]
            if char in dict:
                dict[char] = (dict[char][0] + count, line[i])
            else:
                dict[char] = (count, line[i])
    ...
    for k,v in dict.items():
        dict[k] = (v[0], v[1], v[0]/total, total)  
        ...
        for i in range(len(line) -1):
            ...
            if line[i+1] not in vowel:                                                                                             # consonant
                if line[i] not in vowel:                                                                                           # consonant-to-consonant
                    ...
                    if mem_con == 0:
                        total_weight = FittsLaw(1, m.sprt(2)) + change_layer + click_depth
                    else: 
...
def compute_WPM(data_path, without_weight):
    ...
    while line:
        ...
        for i in range(len(line)):
            ...
            alpha = line[i].lower()
            if alpha in dic:
                dic[alpha] = (dic[alpha][0] + 1,line[i])
            if alpha not in dic:
                dic[alpha] = (1, line[i])
            """
            total_char += 1
    ...
    for k,v in dic.items():
        dic[k] = (v[0], v[1], v[0]/total_char, total_char)  
    print("alphabet_dict=", dic)
    """
...
data_path = 'Ebook3_Gulliver_original_106,015.txt'
memo = memo(data_path)

#print(data_path)

#alpha_dict = alphabetTable(data_path, layout)
#print(alpha_dict)
